...
type CleanupRepository interface {
	...
	DeleteOldLoginAttemptsBatch(retentionDate time.Time, batchSize int) (int64, error)
}
...
func (r *cleanupRepository) DeleteOldLoginAttemptsBatch(retentionDate time.Time, batchSize int) (int64, error) {
	// subquery to safely chunk deletions without locking the entire table
	subQuery := r.db.Table("login_attempts").Select("id").Where("created_at < ?", retentionDate).Limit(batchSize)
	res := r.db.Where("id IN (?)", subQuery).Delete(&models.LoginAttempt{})
	return res.RowsAffected, res.Error
}
